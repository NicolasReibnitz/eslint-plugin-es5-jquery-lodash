...
function isLodash(callee) {
	...
	if (original.type === 'MemberExpression' && original.object.name === '_' && original.property.name === 'chain') {
		return true;
	}
...
function isJQuery(callee) {
	...
	if (
		...
		original.object.type === 'Identifier' &&
		original.object.name[0] === '$'
	) {
		return true;
	}

	// Even in deeply nested objects, we can still check for the jQuery object
	if (
		original.type === 'MemberExpression' &&
		original.object.type === 'MemberExpression' &&
		original.object.property.name[0] === '$'
	) {
		return true;
	}
	...
	create(context) {
		...
		return {
			CallExpression(node) {
				...
				const allFunctions = [].concat(allArrayFunctions, allStringFunctions);
				...
				const baseEs6Functions = [].concat(es6ArrayFunctions, es6StringFunctions, additionalFunctions);
